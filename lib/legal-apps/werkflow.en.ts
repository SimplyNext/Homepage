...
export const werkflowAgbEn: LegalSection[] = [
  ...
  },
  {
    ...
    afterList: [
      "(2) The App is offered as a free trial version (3 free AI quotes and 3 free AI invoices, no credit card required) and in paid subscription plans (“Solo”, “Small Business”, “Pro”). The range of functions of each plan is as shown in the App at the time the contract is concluded. In the trial version, the dictation function (speech to text) is only available as long as free AI quotes or AI invoices remain; once both are used up, dictation is blocked without a subscription, including via the device's speech recognition. With a subscription, dictation remains available even after the monthly quota has been reached.",
