...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: alternatesFor(locale, `/apps/${slug}/agb`),
    robots: noindex,
  };
}
...
export default async function Page({ params }: { params: Promise<Params> }) {
  ...
  return (
    <>
      <InnerPageHeader
        label={t("agb.label")}
        title={t("agb.titleWithApp", { name: app.name })}
        intro={t("agb.introWithApp", { name: app.name })}
        backHref={`/apps/${app.slug}`}
        backLabel={t("backToApp", { name: app.name })}
      />
      <LegalDoc sections={appLegalSections(app.slug, "agb", locale) ?? agbSections(app, locale)} updated={app.since} />
    </>
  );
}
